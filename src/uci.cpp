...
void UCI::loop(int argc, char *argv[])
{
	...
	std::vector<std::istringstream> streamPool;
	...
	do
	{
		do // do-while until queue is empty
		{
			...
			{
				// ::go(pos, is);
				BoardParser::State s = *pos.m_s;
				BoardParser p = BoardParser(pos, &s);
				streamPool.emplace_back(std::istringstream(is.str()));
				threadPool.emplace_back(std::thread(::go, std::ref(pos), std::ref(streamPool.back())));
			}
